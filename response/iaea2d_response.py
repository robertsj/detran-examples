...
inp.put_int("response_spatial_order",     2)
...
db = ResponseDB("iaea.h5")

for m in range(0, 4) :
  # create mesh
  mesh = Mesh2D.Create(fm, fm, cm, cm, [m])
  # create new response and run
  r = Response(inp, mat, mesh, "node"+str(m), db, True, [0.7,0.8,0.85,0.9,0.95,1.0,1.05,1.1,1.15,1.2,1.25,1.3,1.4])
  r.run()
